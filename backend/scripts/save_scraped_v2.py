"""Save corrected scraped data (with comma-handled numbers) and update DB."""
import json
import os
import sys
...
# Corrected data from WeChat backend scrape (commas handled)
articles_raw = [
    ...
    {"title":"从亏损中学习：如何把每次止损变成数据","reads":58,"shares":1,"favorites":10,"likes":0},
    ...
    {"title":"均线系统为什么让你越用越亏？","reads":12,"shares":1,"favorites":1,"likes":1},
    ...
    {"title":"K线经典口诀｜新手炒股避坑指南","reads":11392,"shares":360,"favorites":2199,"likes":168},
...
# Deduplicate by title (keep last occurrence = most recent page)
articles = {}
for a in articles_raw:
    articles[a["title"]] = a
articles = list(articles.values())

print(f"Deduplicated: {len(articles_raw)} -> {len(articles)} articles")
...
try:
    ...
    print(f"Total articles: {total_articles}, Total reads: {total_reads}")
finally:
    db.close()
